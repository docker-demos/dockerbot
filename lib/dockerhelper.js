  ...
  return new Promise(function(resolve, reject) {
    var request = https.request(options, function(response) {
      ...
      response.on('end', function(res) {
        result.body = result.body.replace(/\/n/g,'').replace(/\/t/g,'');
        resolve(result);
      });
    });
...
function scaleApp(state) {
  return marathon.app.update(state.app, configFor())
}

function updateApp(state){
  return marathon.app.update(state.app, configFor(state.app), false);
}
...
function createApp(state){
  return marathon.app.create(configFor(state.app));
}

function configFor(app) {
  var json = require(`/Users/faraaz/Developer/docker/dockerbot/lib/templates/${app}.json`);
  return json;
}
